...
import { buildClearedCookie, buildSessionCookie, hashPassword, } from '../middleware/access.js';
/**
 ...
 * local mode.
 */
export function createAccessRouter(password) {
    ...
    router.get('/', (req, res) => {
        ...
        res
            .status(200)
            .type('html')
            .send(renderAccessPage({
            showError,
            redirect: sanitizeRedirect(rawRedirect),
        }));
    });
    router.post('/', express.urlencoded({ extended: false, limit: '1kb' }), (req, res) => {
        const body = (req.body ?? {});
