...
export function hasValidAccessCookie(req: Request, expectedHash: string): boolean {
 ...
 */
export function buildSessionCookie(req: Request, value: string): string {
  const parts = [
...
export function buildClearedCookie(req: Request): string {
  const parts = [
    `${ACCESS_COOKIE_NAME}=`,
    'Path=/',
    'HttpOnly',
...
function wantsJson(req: Request): boolean {
  if (req.path.startsWith('/api/') || req.path.startsWith('/uploads/')) return true;
 ...
 */
export function createRequireAccess(password: string): RequestHandler {
  if (!password) {
