...
import {
  VIDEO_ACCELERATION,
  type VideoAccelerationMode,
} from '../config/video-processing.config.js';
...
let cachedNvidiaCapabilityPromise: Promise<NvidiaCapability> | null = null;

async function runFfmpegTextCommand(args: string[]): Promise<string> {
...
async function runNvidiaSmokeTest(): Promise<void> {
  await execFileAsync(
    FFMPEG_BIN,
    [
      '-hide_banner',
...
async function detectNvidiaCapability(): Promise<NvidiaCapability> {
  try {
    ...
    return {
      available: true,
      reason:
        'FFmpeg reported CUDA + NVENC support, and the NVIDIA smoke test completed successfully.',
    };
  } catch (error) {
...
async function getNvidiaCapability(): Promise<NvidiaCapability> {
...
export async function resolveProcessingAcceleration(): Promise<AccelerationDecision> {
