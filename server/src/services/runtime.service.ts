...
async function checkBinaryAvailable(binaryName: 'ffmpeg' | 'ffprobe', command: string): Promise<boolean> {
  try {
...
async function pruneStaleEntries(dirPath: string, olderThanMs: number): Promise<number> {
...
export async function runStartupPreflight(): Promise<void> {
  ...
  const [ffmpegAvailable, ffprobeAvailable, removedIncoming, removedThumbnails, removedJobs] =
    await Promise.all([
